"""Generate overseas bridging study Word + PPT deliverables (V2).

Produces two files under ``deliverables/``:
- ``海外桥接-完整模板包.docx`` — full protocol template + regulatory Q&A
- ``海外桥接-细化版汇报.pptx`` — summary presentation

Usage:
    python scripts/common_scripts/build_bridge_docs_v2.py
"""
...
LOG_FORMAT = "%(asctime)s [%(levelname)s] build_bridge_docs: %(message)s"
logging.basicConfig(level=logging.INFO, format=LOG_FORMAT, stream=sys.stderr)
logger = logging.getLogger("build_bridge_docs")

OUT_DIR = Path("deliverables")


# -----------------------------------------------------------------------
# Data: Word document blocks
# -----------------------------------------------------------------------

WORD_BLOCKS: list[tuple[str, list[str]]] = [
    ("1. 研究标题", [
        "一项在[国家/多国家]开展的、随机、盲法、[阳性对照/安慰剂+阳性对照]的临床桥接研究，"
        "用于评估重组带状疱疹疫苗（CHO细胞）在[40岁及以上]人群中的免疫原性与安全性，"
        "并支持既有临床证据的区域外推。",
    ]),
    ("2. 背景与立题依据", [
    ...
    ("3. 研究目的", [
        ...
        "次要目的：评估短期及中期安全性，描述年龄分层（40-49、50-59、≥60、可选≥70）免疫应答差异，"
        "评估体液-细胞免疫一致性。",
    ]),
    ("4. 研究设计", [
        "类型：[单国桥接/多区域桥接]。",
    ...
    ("5. 终点设置", [
        ...
        "次要终点：GMI（gE/VZV）；细胞免疫（gE特异CD4+T细胞应答率/频率）；"
        "征集AE（0-14天）、非征集AE（0-30天）、SAE/AESI（全程）；第2剂后12/24/36月持久性。",
    ]),
    ("6. 统计分析框架", [
...
COUNTRY_QUESTIONS: dict[str, list[str]] = {
    "B. 俄罗斯（含EAEU路径）特异问题": [
...
PPT_SLIDES: list[tuple[str, str, list[str]]] = [
    ("海外桥接策略（细化版）",
...
def _build_word(out_path: Path) -> None:
    """Generate the Word protocol template document."""
    doc = Document()
    ...
    doc.add_paragraph(
        "版本：V2.0\n适用项目：重组带状疱疹疫苗（CHO细胞）\n"
        '说明：本模板按"海外桥接总方案大纲 + 四国监管问题清单"完整展开，'
        "结合国内I/II期、澳洲I期CSR证据与ICH/WHO/EMA通用桥接原则。"
    )

    doc.add_page_break()
    doc.add_heading("模板1：海外桥接总方案大纲（Synopsis/Bridge Study）", 1)

    for title, items in WORD_BLOCKS:
    ...
    for heading, questions in COUNTRY_QUESTIONS.items():
    ...
    doc.add_paragraph(
        "在≥60岁亚组观察到部分免疫指标与阳性对照存在差异。"
        "申办方已预设分层统计、协变量校正及敏感性分析，"
        "并通过SCR、GMC/GMI、细胞免疫与安全性进行综合获益-风险评估；"
        "同时将补充长期持久性及上市后真实世界证据，以降低外推不确定性。"
    )

    try:
        doc.save(out_path)
    except OSError as exc:
        logger.error("failed to save Word document %s: %s", out_path, exc)
        raise
...
def _add_bullet_slide(prs: Presentation, title: str, bullets: Sequence[str]) -> None:
...
def _build_ppt(out_path: Path) -> None:
    """Generate the summary presentation."""
    prs = Presentation()
    for title, subtitle, bullets in PPT_SLIDES:
        if bullets:
    ...
    try:
        ...
        logger.error("failed to save PPT %s: %s", out_path, exc)
        raise
    logger.info("wrote %s", out_path)


def main() -> int:
    """Entry point: generate both deliverables."""
    OUT_DIR.mkdir(exist_ok=True)
    word_path = OUT_DIR / "海外桥接-完整模板包.docx"
    ppt_path = OUT_DIR / "海外桥接-细化版汇报.pptx"

    try:
        _build_word(word_path)
    except OSError:
        return 1
    ...
    # Print output paths for downstream consumption.
    print(word_path)
    print(ppt_path)
    return 0
