...
# 2. Git status
res = subprocess.run(["git", "status", "--short", "--branch"], cwd=cwd, capture_output=True, text=True)
print()
print("=== git status --short --branch ===")
print(res.stdout)

# 3. Last 3 commits on origin/main
res = subprocess.run(["git", "log", "--oneline", "-3", "origin/main"], cwd=cwd, capture_output=True, text=True)
print("=== origin/main last 3 ===")
print(res.stdout)

# 4. Confirm ahead/behind
res = subprocess.run(["git", "rev-list", "--left-right", "--count", "main...origin/main"], cwd=cwd, capture_output=True, text=True)
print("=== ahead/behind ===")
print(res.stdout)
