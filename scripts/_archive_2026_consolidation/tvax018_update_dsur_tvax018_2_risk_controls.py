# -*- coding: utf-8 -*-
"""
《冻干b型流感嗜血杆菌结合疫苗（TVAX-018-2）DSUR#2》优化版后续同步修订：

与 DSUR#1 修订版（四次修订）对齐，将以下两项同步到 DSUR#2 优化版：
1) 19.2 本周期重要风险 段落后新增「重要潜在风险控制措施」表（3 列 × 3 行，
   头=风险分类/风险概述/措施概要；2 行=严重过敏反应、热性惊厥），
   「措施概要」列含「排除标准」「医学干预措施」两个子节；
2) 附件2 重要监管要求汇总表「状态」列升级为与 DSUR#1 修订版一致的详细文本
   （5 条 CDE 临床意见，详细到方案/RMP/ICF 实际条款）。

注意：DSUR#2 为第 2 次 DSUR，19.2 段落实文为「本期报告为本品第2次DSUR…重要的潜在风险维持为…」，
      但风险控制措施表内容与 DSUR#1 完全一致（风险相同：严重过敏反应、热性惊厥）。

依据：
  - 1.8.3 风险管理计划 V1.0（2024年03月05日）
  - 1.3.4.1 1-2期/3期临床试验方案（入选/排除/急救条款）
...
用法：python update_dsur_tvax018_2_risk_controls.py [--apply]
"""
import sys
import docx
...
# ---------------------------------------------------------------------------
# 19.2 表格内容（风险分类 / 风险概述 / 措施概要）—— 与 DSUR#1 完全一致
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 附件2 状态列详细文本（5 条 CDE 临床意见）—— 与 DSUR#1 修订版一致
# ---------------------------------------------------------------------------
ATT2_STATUS = {
    "1.试验设计：":
...
def main(apply=False):
    ...
    for i, p in enumerate(doc.paragraphs):
        if "本期报告为本品第2次DSUR" in p.text:
            target_idx = i
            break
    if target_idx is None:
        print("[ERR] 未找到 19.2 content paragraph（第2次 DSUR）")
        return
