...
def _scan_skills(skills_dir: Path) -> list[SkillInfo]:
    ...
    for skill_dir in sorted(skills_dir.iterdir()):
        ...
        try:
            ...
            # A skill that cannot be read must not abort the whole index build.
            logger.warning("Skipping unreadable skill %s: %s", skill_md.parent.name, exc)
            continue
...
def _generate_index(skills: list[SkillInfo]) -> str:
    ...
    lines: list[str] = [
        ...
        f"<!-- Generated by scripts/_tools/generate_skills_index.py on "
