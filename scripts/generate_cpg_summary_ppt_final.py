...
def create_ppt_standalone(ppt_path):
    ...
    for d in raw_data:
        row_data.append(
            [format_col1(d), d["registry"], d["clinical"], d["safety"], d["ref"]]
        )
    ...
    title_shape = toc_slide.shapes.add_textbox(
    ...
    toc_table_shape = toc_slide.shapes.add_table(
        toc_count + 1, 4, left_margin, Inches(0.9), sum(toc_widths), Inches(5.0)
    )
    ...
    for c_idx, h_text in enumerate(toc_headers):
        ...
        format_cell(
            cell, h_text, size=11, bold=True, text_color=HEADER_TEXT, bg_color=DARK_RED
        )

    r_idx = 1
    for d in raw_data:
        if "在研项目汇总" in d["status"]:
            continue
        ...
        for c_idx, c_text in enumerate(row_fields):
            ...
            format_cell(
                cell, c_text, size=9.5, bold=False, text_color=BLACK, bg_color=bg_color
            )
        r_idx += 1
    ...
    for chunk in slides_data:
        ...
        table_shape = slide.shapes.add_table(
            len(chunk) + 1, 5, left_margin, Inches(0.9), sum(col_widths), Inches(1.0)
        )
    ...
    s_rect = summary_slide.shapes.add_shape(
        pptx.enum.shapes.MSO_SHAPE.RECTANGLE,
        Inches(0),
        Inches(0),
        Inches(13.333),
    ...
    s_title_shape = summary_slide.shapes.add_textbox(
        Inches(0.5), Inches(0.7), Inches(12.33), Inches(0.8)
    )
    ...
    content_box = summary_slide.shapes.add_textbox(
