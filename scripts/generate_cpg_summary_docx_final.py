...
title = doc.add_heading(
    "CpG佐剂预防性疫苗：核心临床试验与安全性数据汇总 (V13)", level=1
)
...
run = subtitle.add_run(
    "本文件汇总了在 FDA 和 NMPA 等登记的含 CpG 佐剂预防性疫苗核心临床数据。融入 FDA 官方审评(Clinical Review)级核心安全性数据。"
)
...
for d in raw_data:
    ...
    clean_status = (
        d["status"].replace("【✅ 已上市】", "已上市").replace("【🧪 在研】", "在研")
    )
...
doc.add_paragraph("")
summary_title = doc.add_heading(
    "【总体安全性总结】基于含 CpG 佐剂预防性疫苗临床数据的综合评价", level=2
)
