...
pipeline = make_pipeline(StandardScaler(), PCA(n_components=10), SVC(kernel="rbf"))
...
pipeline.set_params(pca__n_components=15, classifier__C=0.1)
...
full_pipeline = Pipeline([("preprocessor", preprocessor), ("classifier", LogisticRegression())])
...
# By column indices
preprocessor = ColumnTransformer(
    [("num", StandardScaler(), [0, 1, 2]), ("cat", OneHotEncoder(), [3, 4])]
)
...
preprocessor = ColumnTransformer(
    [("num", StandardScaler(), numeric_mask), ("cat", OneHotEncoder(), categorical_mask)]
)
...
preprocessor = ColumnTransformer([("num", StandardScaler(), is_numeric)])
```
...
feature_union = FeatureUnion([("pca", PCA(n_components=10)), ("select_best", SelectKBest(k=20))])
...
feature_union = FeatureUnion(
    [("pca", PCA(n_components=10)), ("svd", TruncatedSVD(n_components=10))]
)
...
feature_union = FeatureUnion(
    transformer_list=[("pca", PCA(n_components=10)), ("select_best", SelectKBest(k=20))],
    transformer_weights={
...
full_pipeline = Pipeline(
    [
        ("features", FeatureUnion([("text", text_pipeline), ("numeric", StandardScaler())])),
        ("classifier", LogisticRegression()),
    ]
)
```
...
model = TransformedTargetRegressor(
    regressor=LinearRegression(), transformer=QuantileTransformer(output_distribution="normal")
)
...
cachedir = mkdtemp()
pipeline = Pipeline(
    [("expensive_preprocessing", ExpensiveTransformer()), ("classifier", LogisticRegression())],
    memory=cachedir,
)
```
