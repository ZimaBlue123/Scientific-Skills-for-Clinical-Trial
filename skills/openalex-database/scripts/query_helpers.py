...
def find_author_works(
    author_name: str, client: OpenAlexClient, limit: int | None = None
) -> list[dict[str, Any]]:
    """
...
def find_institution_works(
    institution_name: str, client: OpenAlexClient, limit: int | None = None
) -> list[dict[str, Any]]:
    """
...
def analyze_research_output(
    ...
    # Step 1: Find entity
    entity_response = client._make_request(endpoint, params={"search": entity_name, "per-page": 1})
    ...
    # OA percentage
    oa_works = client.search_works(filter_params={**filter_params, "is_oa": "true"}, per_page=1)
