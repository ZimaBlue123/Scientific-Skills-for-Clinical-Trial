...
def format_continuous_variable(data, variable, group_col, distribution="normal"):
    ...
    results["p_value"] = (
        f"{p_value:.3f}" if p_value < 0.001 else f"{p_value:.2f}" if p_value < 1.0 else "—"
    )
...
def generate_baseline_table(data, group_col="group", output_file="table1_baseline.csv"):
    ...
    if "ecog_ps" in data.columns:
        table_rows.append(
            {"Characteristic": "ECOG PS, n (%)", **{g: "" for g in groups}, "p_value": ""}
        )
...
def generate_efficacy_table(data, group_col="group", output_file="table2_efficacy.csv"):
    ...
    if "best_response" in data.columns:
        for group in groups:
            group_data = data[data[group_col] == group]
            cr_pr = (
                (group_data["best_response"] == "CR") | (group_data["best_response"] == "PR")
            ).sum()
...
def main():
    ...
    parser.add_argument(
        "-o", "--output-dir", type=str, default="tables", help="Output directory (default: tables)"
    )
    parser.add_argument(
        "--group-col", type=str, default="group", help="Column name for grouping variable"
    )
