...
def calculate_hazard_ratio(
    ...
    cph = CoxPHFitter()
    cph.fit(
        data_cox[[time_col, event_col, "group_binary"]], duration_col=time_col, event_col=event_col
    )
...
def main():
    ...
    parser.add_argument(
        "-t", "--title", type=str, default="Kaplan-Meier Survival Curve", help="Plot title"
    )
