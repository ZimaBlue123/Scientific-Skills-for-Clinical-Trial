...
agg_result = (
    ddf.groupby("category").agg({"value": ["mean", "sum", "count"], "amount": "sum"}).compute()
)
```
...
ddf = ddf.map_partitions(
    func, meta=pd.DataFrame({"col1": pd.Series(dtype="float64"), "col2": pd.Series(dtype="int64")})
)
```
...
result1, result2, result3 = dask.compute(operation1, operation2, operation3)
```
...
# Aggregate
summary = ddf.groupby("category").agg({"amount": ["sum", "mean"], "quantity": "count"})
