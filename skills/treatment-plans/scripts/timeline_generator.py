...
def create_visual_timeline(timeline_data: dict, output_file: Path, start_date: str = None):
    ...
    if not HAS_MATPLOTLIB:
        print(
            "Error: matplotlib not installed. Install with: pip install matplotlib", file=sys.stderr
        )
    ...
    legend_elements = [
        Rectangle(
            (0, 0), 1, 1, fc="steelblue", alpha=0.7, edgecolor="black", label="Treatment Phase"
        ),
        Line2D(
            [0],
            [0],
...
Note: Visual timeline generation requires matplotlib.
  Install with: pip install matplotlib
    ...
    parser.add_argument(
        "--plan", type=Path, required=True, help="Treatment plan file to analyze (.tex format)"
    )
    ...
    parser.add_argument(
        "--visual", action="store_true", help="Generate visual timeline (requires matplotlib)"
    )
