...
with pm.Model() as anova_model:
    ...
    group_means = pm.Normal("group_means", mu=mu_global, sigma=sigma_between, shape=n_groups)
...
with pm.Model() as corr_model:
    ...
    obs = pm.MvNormal("obs", mu=[0, 0], cov=cov_matrix, observed=np.column_stack([x, y]))
