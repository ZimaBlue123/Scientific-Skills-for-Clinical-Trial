...
def check_normality(
    data: np.ndarray | pd.Series | list, name: str = "data", alpha: float = 0.05, plot: bool = True
) -> dict:
    """
...
def check_linearity(
    x: np.ndarray | pd.Series, y: np.ndarray | pd.Series, x_name: str = "X", y_name: str = "Y"
) -> dict:
    """
...
def comprehensive_assumption_check(
    data: pd.DataFrame, value_col: str, group_col: str | None = None, alpha: float = 0.05
) -> dict:
    """
