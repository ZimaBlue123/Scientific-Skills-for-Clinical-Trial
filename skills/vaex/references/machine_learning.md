...
# Custom range
minmax_scaler = vaex.ml.MinMaxScaler(features=["age"], feature_range=(-1, 1))
```
...
# Control prefix
onehot = vaex.ml.OneHotEncoder(features=["category"], prefix="cat_")
```
...
target_encoder = vaex.ml.TargetEncoder(features=["category"], target="target_variable")
...
woe_encoder = vaex.ml.WeightOfEvidenceEncoder(features=["category"], target="binary_target")
woe_encoder.fit(df)
...
pca = vaex.ml.PCA(features=["feature1", "feature2", "feature3", "feature4"], n_components=2)
pca.fit(df)
df = pca.transform(df)
...
projector = vaex.ml.RandomProjection(features=["x1", "x2", "x3", "x4", "x5"], n_components=3)
projector.fit(df)
...
```python
# Cluster data
kmeans = vaex.ml.KMeans(features=["feature1", "feature2", "feature3"], n_clusters=5, max_iter=100)
kmeans.fit(df)
...
sklearn_model = vaex.ml.sklearn.Predictor(
    features=features, target=target, model=model, prediction_name="rf_prediction"
)
...
booster = vaex.ml.xgboost.XGBoostModel(features=features, target=target, prediction_name="xgb_pred")

# Configure parameters
params = {"max_depth": 6, "eta": 0.1, "objective": "reg:squarederror", "eval_metric": "rmse"}

# Train
booster.fit(df=train_df, params=params, num_boost_round=100)
...
lgb_model = vaex.ml.lightgbm.LightGBMModel(
    features=features, target=target, prediction_name="lgb_pred"
)
...
# Train
lgb_model.fit(df=train_df, params=params, num_boost_round=100)
...
catboost_model = vaex.ml.catboost.CatBoostModel(
    features=features, target=target, prediction_name="catboost_pred"
)

# Parameters
params = {"iterations": 100, "depth": 6, "learning_rate": 0.1, "loss_function": "Logloss"}
...
# Train
keras_model.fit(train_df, epochs=10, batch_size=10000)
...
features = [
    "label_encoded_cat1",
    "label_encoded_cat2",
    "standard_scaled_num1",
    "standard_scaled_num2",
    "standard_scaled_num3",
]
model = vaex.ml.sklearn.Predictor(
    features=features, target="target", model=RandomForestClassifier(n_estimators=100)
)
model.fit(train)
