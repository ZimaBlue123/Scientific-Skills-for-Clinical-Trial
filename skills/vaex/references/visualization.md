...
df_sample.scatter(df_sample.x, df_sample.y, alpha=0.5, s=10)  # Point size
...
# Mean on a grid
df.plot(
    df.x, df.y, what=df.z.mean(), limits=[[0, 10], [0, 10]], shape=(100, 100), colormap="viridis"
)
...
# Count
df.plot(df.x, df.y, what="count", ax=axes[0, 0], show=False)
axes[0, 0].set_title("Count")

# Mean
df.plot(df.x, df.y, what=df.z.mean(), ax=axes[0, 1], show=False)
axes[0, 1].set_title("Mean of z")

# Std
df.plot(df.x, df.y, what=df.z.std(), ax=axes[1, 0], show=False)
axes[1, 0].set_title("Std of z")

# Min
df.plot(df.x, df.y, what=df.z.min(), ax=axes[1, 1], show=False)
...
df.plot(df.x, df.y, selection="group_a", ax=ax, show=False, colormap="Reds", alpha=0.5)
df.plot(df.x, df.y, selection="group_b", ax=ax, show=False, colormap="Blues", alpha=0.5)
...
for idx, cat in enumerate(categories):
    ...
    df_cat.plot(df_cat.x, df_cat.y, ax=axes[idx], show=False)
...
df.plot(df.x, df.y, limits="99%", shape=(256, 256), colormap="plasma", ax=ax, show=False)
...
df.plot(df.x, df.y, figsize=(12, 10), dpi=300)
```
...
plt.hexbin(df.x.values[:100000], df.y.values[:100000], gridsize=50, cmap="viridis")
...
mean_vx = df.mean(df.vx, binby=[df.x, df.y], limits=[[0, 10], [0, 10]], shape=(20, 20))
mean_vy = df.mean(df.vy, binby=[df.x, df.y], limits=[[0, 10], [0, 10]], shape=(20, 20))
...
counts = df.count(binby=[df.x, df.y], limits=[[0, 10], [0, 10]], shape=(512, 512))
...
for idx, cat in enumerate(categories):
    ...
    # 1D histogram
    df.plot1d(df.value, selection=f"cat_{cat}", ax=axes[idx, 0], show=False)
    axes[idx, 0].set_title(f"Category {cat} - Distribution")

    # 2D plot
    df.plot(df.x, df.y, selection=f"cat_{cat}", ax=axes[idx, 1], show=False)
