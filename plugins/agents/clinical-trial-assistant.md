...
|--------|--------------|----------------|
| `scripts/office_tools/extract_office_utils.py` | `from scripts.office_tools.extract_office_utils import extract_docx_text` | Do NOT import `extract_text` — the function is `extract_docx_text` |
| `scripts/common_scripts/docx_utils.py` | `apply_cn_en_fonts(doc)` after all content is written | Calling before content insertion resets font on empty paragraphs |
| `scripts/common_scripts/generator_base.py` | `make_argparser()` returns parser, call `.parse_args()` separately | Do NOT call `parse_args()` inside `make_argparser()` |
| `scripts/common_scripts/clinical_rag_utils.py` | `ClinicalDocumentIndex(chunk_size=500)` | Default chunk_size is 200; use 500 for clinical PDFs |
| `scripts/pptx_tools/check_pptx_overflow.py` | Run AFTER generating PPTX, BEFORE delivering to user | Overflow errors in delivered decks waste revision cycles |
